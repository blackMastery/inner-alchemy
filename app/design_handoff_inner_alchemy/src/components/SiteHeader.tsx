...
export default function SiteHeader() {
  ...
  return (
    <header className="sticky top-0 z-50 border-b border-rule bg-linen/95 backdrop-blur-sm">
      <div className="mx-auto flex max-w-[1120px] items-center justify-between gap-8 px-8 py-[18px] max-md:px-[22px]">
        <Link href="/" className="flex flex-col gap-0.5">
          <span className="flex items-baseline gap-2.5 font-display text-[26px] font-medium tracking-[0.04em] text-body">
            Inner <span className="text-[15px] text-clay">✦</span> Alchemy
          </span>
          <span className="text-[9.5px] uppercase tracking-[0.24em] text-muted-2">
            ...
            return (
              <Link
                ...
                className={`border-b pb-[3px] text-sm font-semibold ${
                  active ? "border-clay-light text-clay" : "border-transparent text-body-2 hover:text-clay"
                }`}
              >
                {item.label}
              </Link>
      ...
      {menuOpen && (
        <div className="flex flex-col border-t border-rule bg-linen px-[22px] pb-[22px] pt-3.5 lg:hidden">
          {NAV.map((item) => (
            <Link
              key={item.href}
              href={item.href}
              ...
              className={`border-b border-rule-4 py-3.5 text-[17px] font-semibold ${
                pathname === item.href ? "text-clay" : "text-body"
              }`}
            >
              {item.label}
            </Link>
