...
import { PRACTITIONER, PROGRAMS, PRICING_ROWS, TESTIMONIALS, FIT } from "@/content/site";
  ...
  return (
    <>
      ...
      <Section className="grid grid-cols-[1.15fr_0.85fr] items-center gap-[72px] !pb-[88px] !pt-[104px] max-md:grid-cols-1 max-md:gap-8">
        <div>
          ...
          <div className="flex flex-wrap items-center gap-6">
            <BookingButton />
            <Link href="/session" className="border-b border-sage-light pb-0.5 text-[15px] font-semibold text-sage hover:text-sage-dark">
              What a session actually looks like →
            </Link>
          </div>
          <p className="mt-5 text-[13px] text-muted-2">No obligation. It&rsquo;s a conversation, not a sales call.</p>
        </div>
      ...
      <Section tone="warm" className="grid grid-cols-[0.9fr_1.1fr] gap-[72px] max-md:grid-cols-1 max-md:gap-8">
        <div>
          <Eyebrow>No mystery, no surprises</Eyebrow>
        ...
        <div className="flex flex-col">
          {[
            ...
            >
              <p className="font-display text-[22px] text-clay">{span}</p>
              <div>
        ...
        <div className="mx-auto grid max-w-[960px] grid-cols-2 gap-8 max-md:grid-cols-1">
          <Card className="!p-10 max-md:!p-[26px]">
            ...
            <ul className="flex flex-col gap-4 text-[15px] leading-[1.65] text-body-3">
              {FIT.yes.map((line) => (
                <li key={line} className="flex gap-3"><span className="text-sage">✦</span><span>{line}</span></li>
              ))}
            </ul>
          </Card>
          <Card className="!p-10 max-md:!p-[26px]">
            ...
            <ul className="flex flex-col gap-4 text-[15px] leading-[1.65] text-body-3">
              {FIT.no.map((line) => (
                <li key={line} className="flex gap-3"><span className="text-[#C09A82]">—</span><span>{line}</span></li>
              ))}
            </ul>
          </Card>
        </div>
        <p className="mx-auto mt-8 max-w-[60ch] text-center text-sm text-muted-2">
        ...
        <div className="grid grid-cols-3 gap-7 max-md:grid-cols-1">
          {TESTIMONIALS.map((t) => (
            <figure key={t.attribution} className="flex flex-col gap-5 rounded-[18px] border border-rule/20 bg-linen/[0.06] p-9 max-md:p-[26px]">
              <blockquote className="font-display text-xl italic leading-[1.55] text-[#EFE7D8]">&ldquo;{t.quote}&rdquo;</blockquote>
          ...
          {ladder.map((p, i) => {
            ...
            return (
              ...
              >
                ...
                <p className={`text-sm font-semibold ${dark ? "text-clay-light" : "text-clay"}`}>
                  {p.price} · {p.duration}
                </p>
              </div>
            );
        ...
        <div className="overflow-hidden rounded-[18px] border border-rule-2 bg-parchment">
          {PRICING_ROWS.map((row, i) => (
            ...
            >
              ...
              <span className={`font-bold ${row.price === "Free" ? "text-sage" : row.highlight ? "text-lg text-clay" : "text-body"}`}>
                {row.price}
              </span>
            </div>
          ))}
        </div>
        <p className="mt-[22px] text-center text-[13px] text-muted-2">
